...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemplng'])){
        if(is_uploaded_file($_FILES['file_templatemplng']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemplng'];                 
            ...
            $nama_file2 = $userhris."-mplng-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmplng = trim($value[1]);
            $labelmplng = trim($value[2]);
