...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemdplk'])){
        if(is_uploaded_file($_FILES['file_templatemdplk']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemdplk'];                 
            ...
            $nama_file2 = $userhris."-mdplk-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $jenis_dplk = trim($value[1]);
            
            $sql = "insert into m_jenis_dplk(jenis_dplk) values('$jenis_dplk')";
