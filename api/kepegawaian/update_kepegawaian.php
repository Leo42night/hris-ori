...
if ($userhris){
    ...
    // $person_grade = addslashes($_REQUEST['person_gradepegawai']);
    if($person_grade=="-"){
        $person_grade = "";
    }
    ...
    if ($result){        
        ...
        $sql2 .= " on duplicate key update tgl_lahir='$tgl_lahir',jenis_kelamin='$jenis_kelamin',person_grade='$person_grade',agama='$id_agama',nik='$nik',npwp='$npwp',tgl_masuk='$tgl_masuk',tgl_capeg='$tgl_capeg',tgl_tetap='$tgl_tetap',keterangan_mutasi='$keterangan_mutasi',tahun_pengangkatan='$tahun_pengangkatan',kode_pln_group='$kode_pln_group'";    
        // $sql2 = " on duplicate key update tgl_lahir='$tgl_lahir',jenis_kelamin='$jenis_kelamin',person_grade='$person_grade'";    
        $result2 = @mysqli_query($koneksi,$sql2);
        if($result2){
