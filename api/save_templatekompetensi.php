...
if ($userhris){
    ...
    if(isset($_FILES['file_templatekompetensi'])){
        if(is_uploaded_file($_FILES['file_templatekompetensi']['tmp_name'])){
            $uploadFile = $_FILES['file_templatekompetensi'];                 
            ...
            $nama_file2 = $userhris."-kompetensi-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            ...
            $nip = trim($value[2]);        
            $cluster = trim($value[3]);
            $kompetensi = trim($value[4]);
            $rating = trim($value[5]);
            $deskripsi = trim($value[6]);
            $presentase = trim($value[7]);

            $start_date = TanggalIndo2($start_date2);
            $end_date = TanggalIndo2($end_date2);

            if($nip!==""){
                $rs2 = mysqli_query($koneksi,"select * from r_kompetensi where nip='$nip' and start_date='$start_date' and end_date='$end_date'");
                $jumlah_data2 = mysqli_num_rows($rs2);            
                if($jumlah_data2==0){  
                    $sql = "insert into r_kompetensi";
                    $sql .= "(nip,start_date,end_date,cluster,kompetensi,rating,deskripsi,presentase)";    
                    $sql .= " values('$nip','$start_date','$end_date','$cluster','$kompetensi','$rating','$deskripsi','$presentase')";
                    ...
                    if($presentase!=""){
                        if($sql==""){
                            $sql .= "set presentase='$presentase'";
                        } else {
                            $sql .= ",presentase='$presentase'";
                        }                    
                    }
                    if($sql!=""){
                        $sql2 = "update r_kompetensi ";
                        $sql2 .= $sql;
                        $sql2 .= " where nip='$nip'"; 
