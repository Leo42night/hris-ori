...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemkabkot'])){
        if(is_uploaded_file($_FILES['file_templatemkabkot']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemkabkot'];                 
            ...
            $nama_file2 = $userhris."-mkabkot-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmkabupaten = trim($value[1]);
            $idmprovinsi = trim($value[2]);
            $labelmkabkot = trim($value[3]);
            
            $sql = "insert into m_kabupaten(id_kabupaten, id_provinsi, nama_kabupaten) values('$idmkabupaten','$idmprovinsi','$labelmkabkot')";
