...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemjpend'])){
        if(is_uploaded_file($_FILES['file_templatemjpend']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemjpend'];                 
            ...
            $nama_file2 = $userhris."-mjpend-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmjpend = trim($value[1]);
            $labelmjpend = trim($value[2]);
