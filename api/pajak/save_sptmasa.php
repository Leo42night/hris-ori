        ...
        if($blth<$tahun."-12"){
            ...
            while($hasil4 = mysqli_fetch_array($rs4)){
                ...
                if($hasil3){
                    ...
                    $non_rutin_penyesuaian = floatval($hasil3['non_rutin_penyesuaian']);
                } else {
                    $cop = 0;
                    $fasilitas_hp = 0;
                    ...
                    $medical_checkup = 0;
                    $non_rutin_penyesuaian = 0;
                }
                $natuna = $sppd+$cop+$fasilitas_hp+$reimburse_kesehatan+$asuransi_kesehatan+$sarana_kerja+$sppd_manual+$asuransi_purna_jabatan+$medical_checkup+$non_rutin_penyesuaian;
            ...
            while($hasil4 = mysqli_fetch_array($rs4)){
                ...
                $rs3 = mysqli_query($koneksi,"select * from perhitungan_pajak_khusus where nip='$nip'");
                $hasil3 = mysqli_fetch_array($rs3);
                if($hasil3){
                    $status_dekom = 1;
                } else {
                ...
                // $tantiem = 0;                
                $rs3 = mysqli_query($koneksi,"SELECT sum(tantiem) as tantiem,sum(pph21) as pph21_tantiem from tantiem where nip='$nip' and blth>='$blth_awal' and blth<='$blth_akhir'");
                $hasil3 = mysqli_fetch_array($rs3);
                if($hasil3){
                    $tantiem = floatval($hasil3['tantiem']);
                    $pph21_tantiem = floatval($hasil3['pph21_tantiem']);
                } else {
                    $tantiem = 0;
                    $pph21_tantiem = 0;
                }
                ...
                $rs3 = mysqli_query($koneksi,"SELECT sum(cop) as cop,sum(fasilitas_hp) as fasilitas_hp,sum(reimburse_kesehatan) as reimburse_kesehatan,sum(asuransi_kesehatan) as asuransi_kesehatan,sum(sarana_kerja) as sarana_kerja,sum(sppd_manual) as sppd_manual,sum(asuransi_purna_jabatan) as asuransi_purna_jabatan,sum(medical_checkup) as medical_checkup,sum(non_rutin_penyesuaian) as non_rutin_penyesuaian from natura where nip='$nip' and blth>='$blth_awal' and blth<='$blth_akhir'");
                $hasil3 = mysqli_fetch_array($rs3);
                if($hasil3){
                    $cop = floatval($hasil3['cop']);
                    $fasilitas_hp = floatval($hasil3['fasilitas_hp']);
                ...
                $biaya_jabatanb = 0;
                $iuran_pensiunb = 0;
                $brutot = $gaji+$honorarium+$tunjangan_variable+$benefit2+$natuna+$beban_pph21+$bonus_thr;
                if($status_dekom==0){
                ...
                if($hasil3){
                    ...
                    $non_rutin_penyesuaian_mapping = floatval($hasil3['non_rutin_penyesuaian']);
                } else {
                    $cop_mapping = 0;
                    ...
                    $non_rutin_penyesuaian_mapping = 0;
                }
                $natuna_mapping = $sppd_mapping+$cop_mapping+$fasilitas_hp_mapping+$reimburse_kesehatan_mapping+$asuransi_kesehatan_mapping+$sarana_kerja_mapping+$sppd_manual_mapping+$asuransi_purna_jabatan_mapping+$medical_checkup_mapping+$non_rutin_penyesuaian_mapping;
                ...
                $nettot_akhir = $nettot+$nettot_sebelumnya;
                $ppht_sebelumnya = $ppht_sebelumnya2+$ppht_sebelumnya1+$pph21_tantiem;
                ...
                if($status_dekom==0){ 
                    $ptkp = $ptkp;                   
                    $biaya_jabatant = $biaya_jabatant;
                    $iuran_pensiunt = $iuran_pensiunt;
                    $biaya_pengurangt = $biaya_pengurangt;
                    $ptkp = $ptkp;
                ...
                if ($result1){
                    if($ppht_terutang2!=0){
                        $sql3 = "insert into kelebihan_bayar_rampung(blth,nip,ppht,ppht_sebelumnya,ppht_mutasi,ppht_tantiem,selisih,blthnip)";
                        $sql3 .= " values('$blth','$nip','$ppht','$ppht_sebelumnya1','$ppht_sebelumnya2','$pph21_tantiem','$ppht_terutang2','$blthnip')";
