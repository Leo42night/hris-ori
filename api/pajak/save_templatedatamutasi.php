...
if ($userhris){
    ...
    if(isset($_FILES['file_templatedatamutasi'])){
        if(is_uploaded_file($_FILES['file_templatedatamutasi']['tmp_name'])){
            $uploadFile = $_FILES['file_templatedatamutasi'];                 
            ...
            $nama_file2 = $userhris."-datamutasi-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $nip = trim($value[1]);
            $tahun = trim($value[2]);
            $blth_awal = trim($value[3]);
            $blth_akhir = trim($value[4]);
            $netto = floatval($value[5]);
            $pph21 = floatval($value[6]);
            $niptahun = $tahun.".".$nip;
        
            if($nip!=="" && $tahun!=="" && $blth_awal!=="" && $blth_akhir!=="" && ($netto>0 || $pph21>0)){
