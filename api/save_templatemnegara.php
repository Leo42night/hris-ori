...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemnegara'])){
        if(is_uploaded_file($_FILES['file_templatemnegara']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemnegara'];                 
            ...
            $nama_file2 = $userhris."-mnegara-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $kodemnegara = trim($value[1]);
            $namamnegara = trim($value[2]);
