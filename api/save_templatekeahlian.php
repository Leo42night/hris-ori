...
if ($userhris){
    ...
    if(isset($_FILES['file_templatekeahlian'])){
        if(is_uploaded_file($_FILES['file_templatekeahlian']['tmp_name'])){
            $uploadFile = $_FILES['file_templatekeahlian'];                 
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            ...
            $nip = trim($value[2]);        
            $kode_profesi = trim($value[3]);
            $tingkat_keahlian = trim($value[4]);

            $start_date = TanggalIndo2($start_date2);
            $end_date = TanggalIndo2($end_date2);

            if($nip!==""){
                $rs2 = mysqli_query($koneksi,"select * from r_keahlian where nip='$nip' and start_date='$start_date' and end_date='$end_date'");
                ...
                if($jumlah_data2==0){  
                    ...
                    if($tingkat_keahlian!=""){
                        if($sql==""){
                            $sql .= "set tingkat_keahlian='$tingkat_keahlian'";
                        } else {
                            $sql .= ",tingkat_keahlian='$tingkat_keahlian'";
                        }                    
                    }
                    if($sql!=""){
                        $sql2 = "update r_keahlian ";
                        $sql2 .= $sql;
                        $sql2 .= " where nip='$nip'"; 
