...
if ($userhris){
    ...
    if(isset($_FILES['file_templatekaryatulis'])){
        if(is_uploaded_file($_FILES['file_templatekaryatulis']['tmp_name'])){
            $uploadFile = $_FILES['file_templatekaryatulis'];                 
            ...
            $nama_file2 = $userhris."-karyatulis-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            ...
            $nip = trim($value[2]);
            $judul = trim($value[3]);
            $media_publikasi = trim($value[4]);
            $tahun = trim($value[5]);
            $kode_jenis = trim($value[6]);

            $start_date = TanggalIndo2($start_date2);
            $end_date = TanggalIndo2($end_date2);

            if($nip!==""){
                $rs2 = mysqli_query($koneksi,"select * from r_karya_tulis where nip='$nip' and start_date='$start_date' and end_date='$end_date'");
                ...
                if($jumlah_data2==0){  
                    ...
                    if($media_publikasi!=""){
                        if($sql==""){
                            $sql .= "set media_publikasi='$media_publikasi'";
                        } else {
                            $sql .= ",media_publikasi='$media_publikasi'";
                        }                    
                    }
                    if($tahun!=""){
                        if($sql==""){
                    ...
                    if($kode_jenis!=""){
                        if($sql==""){
                            $sql .= "set kode_jenis='$kode_jenis'";
                        } else {
                            $sql .= ",kode_jenis='$kode_jenis'";
                        }                    
                    }
                    if($sql!=""){
                        $sql2 = "update r_karya_tulis ";
                        $sql2 .= $sql;
                        $sql2 .= " where nip='$nip'"; 
