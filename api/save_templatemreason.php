...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemreason'])){
        if(is_uploaded_file($_FILES['file_templatemreason']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemreason'];                 
            ...
            $nama_file2 = $userhris."-mreason-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmreason = trim($value[1]);
            $labelmreason = trim($value[2]);
