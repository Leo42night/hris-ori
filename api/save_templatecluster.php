...
if ($userhris){
    ...
    if(isset($_FILES['file_templatecluster'])){
        if(is_uploaded_file($_FILES['file_templatecluster']['tmp_name'])){
            $uploadFile = $_FILES['file_templatecluster'];                 
            ...
            $nama_file2 = $userhris."-cluster-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            ...
            $cluster = trim($value[4]);

            $start_date = TanggalIndo2($start_date2);
            $end_date = TanggalIndo2($end_date2);

            if($nip!==""){
                $rs2 = mysqli_query($koneksi,"select * from r_cluster where nip='$nip' and start_date='$start_date' and end_date='$end_date'");
