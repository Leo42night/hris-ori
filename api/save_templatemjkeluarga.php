...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemjkeluarga'])){
        if(is_uploaded_file($_FILES['file_templatemjkeluarga']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemjkeluarga'];                 
            ...
            $nama_file2 = $userhris."-mjkeluarga-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmjkeluarga = trim($value[1]);
            $labelmjkeluarga = trim($value[2]);
