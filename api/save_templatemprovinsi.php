...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemprovinsi'])){
        if(is_uploaded_file($_FILES['file_templatemprovinsi']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemprovinsi'];                 
            ...
            $nama_file2 = $userhris."-mprovinsi-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmprovinsi = trim($value[1]);
            $labelmprovinsi = trim($value[2]);
