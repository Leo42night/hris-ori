...
if ($userhris){
    ...
    $blth_ini = date("Y-m"); 
    $today = date("Y-m-d");
    $nip2 = isset($_POST['niprcuticari']) ? $_POST['niprcuticari'] : "";
    $perintah = "";
    if($nip2!=""){
    ...
    $offset = ($page-1)*$rows;
    $result = array();
    $rs = mysqli_query($koneksi,"select count(a.id) from cuti a inner join data_pegawai b on a.nip=b.nip where b.aktif='1'".$perintah);
