...
if ($userhris){
    ...
    $rs99 = mysqli_query($koneksi,"SELECT id,tgl_awal,tgl_akhir FROM ijin where nip='$nip' and id<>'$id' and ('".$tgl_awal."' BETWEEN tgl_awal and tgl_akhir or '".$tgl_akhir."' BETWEEN tgl_awal and tgl_akhir)");
    ...
    if($id_cuti==0 && $id_ijin==0 && $id_sppd==0){   
        if($hari>=3 && $eviden==""){
            ...
            $sql = "update ijin set jenis_ijin='$jenis_ijin',tgl_awal='$tgl_awal',tgl_akhir='$tgl_akhir',hari='$hari',alasan_ijin='$alasan_ijin'";
            if($eviden!=""){
                $sql .= ",eviden='$eviden'";
            }
            $sql .= " where id=$id";
