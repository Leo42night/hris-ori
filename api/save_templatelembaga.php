...
if ($userhris){
    ...
    if(isset($_FILES['file_templatelembaga'])){
        if(is_uploaded_file($_FILES['file_templatelembaga']['tmp_name'])){
            $uploadFile = $_FILES['file_templatelembaga'];                 
            ...
            $nama_file2 = $userhris."-lembaga-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            ...
            $nip = trim($value[2]);
            $nama_organisasi = trim($value[3]);
            $jabatan = trim($value[4]);
            $uraian_kegiatan = trim($value[5]);
            $kode_organisasi = trim($value[6]);

            $start_date = TanggalIndo2($start_date2);
            $end_date = TanggalIndo2($end_date2);

            if($nip!==""){
                $rs2 = mysqli_query($koneksi,"select * from r_lembaga where nip='$nip' and start_date='$start_date' and end_date='$end_date'");
                ...
                if($jumlah_data2==0){  
                    ...
                    if($nama_organisasi!=""){
                        if($sql==""){
                            $sql .= "set nama_organisasi='$nama_organisasi'";
                        } else {
                    ...
                    if($uraian_kegiatan!=""){
                        if($sql==""){
                            $sql .= "set uraian_kegiatan='$uraian_kegiatan'";
                        } else {
                            $sql .= ",uraian_kegiatan='$uraian_kegiatan'";
                        }                    
                    }
                    if($kode_organisasi!=""){
                        if($sql==""){
                            $sql .= "set kode_organisasi='$kode_organisasi'";
                        } else {
                            $sql .= ",kode_organisasi='$kode_organisasi'";
                        }                    
                    }
                    if($sql!=""){
                        $sql2 = "update r_lembaga ";
                        $sql2 .= $sql;
                        $sql2 .= " where nip='$nip'"; 
