...
if ($userhris){
    ...
    if(isset($_FILES['file_templatemagama'])){
        if(is_uploaded_file($_FILES['file_templatemagama']['tmp_name'])){
            $uploadFile = $_FILES['file_templatemagama'];                 
            ...
            $nama_file2 = $userhris."-magama-".date('YmdHis').".$fileExt";
    ...
    if ($nama_file2!=""){
        ...
        foreach ($rows as $key => $value) {
            $idmagama = trim($value[1]);
            $labelmagama = trim($value[2]);
