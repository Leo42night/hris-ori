...
if ($userhris){
    ...
    $items = array();
    $rs = mysqli_query($koneksi,"select a.*,b.proses,b.lihat from nodes a left join aksesuser b ON a.id=b.idmenu and b.username='$userhris' and (b.proses='1' or b.lihat='1') where a.grup='HXMS-API' and a.parentId<>'' and a.parentId2='' order by a.urut,a.id asc");
    while ($hasil = mysqli_fetch_array($rs)) {
        ...
        $proses = $hasil['proses'];
        $lihat = $hasil['lihat'];
        $filter = $name."|hxms-api/".$url."|".$proses."|".$lihat;
        ...
        if($cekdata>0){
            ...
            while ($hasil2 = mysqli_fetch_array($rs2)) {
                ...
                $lihat2 = $hasil2['lihat'];
                $filter2 = $name2."|hxms-api/".$url2."|".$proses2."|".$lihat2;
