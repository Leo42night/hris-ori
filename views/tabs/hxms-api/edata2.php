...
if (!$userhris || ($akses_proses!="1" && $akses_view!="1")){
    ...
    <table id="dgedata2" title="" class="easyui-datagrid" style="width:100%;height:100%"	
            url="<?=$foldernya;?>get_master_edata2.php" pageSize="20"        
    ...
    <script>
        $('#dgedata2').datagrid({
            ...
            onExpandRow: function(index,row){   
                ...
                ddv.datagrid({
                    url:'<?=$foldernya;?>get_detail_edata2.php?nama_tabel='+row.nama_tabeledata2+'&blth='+row.blthedata2,
                    fitColumns:true,
                    singleSelect:true,
                    pagination:false,
                    nowrap:false,
    ...
    <script type="text/javascript">
        ...
        function gettoken(){
            $.messager.confirm('Konfirmasi','Proses request token?',function(r){
                if (r){
                    ...
                    $.post('<?=$foldernya;?>req_token.php',{},function(result){
                        //alert(result);
        ...
        function kirimedata2(index){
    		...
    		if (row){
                ...
                url = 'update_edata.php[not_found]?id='+row.idedata;  
                */
            }          
        }        
    		...
    		if (jumlah>0){
    			$.messager.confirm('Konfirmasi','Export jenis data yang dipilih?',function(r){
    				if (r){
    					...
    					$.post('<?=$foldernya;?>proses_edata.php',{ids2:ids2,blth:blthnya},function(result){
                            //alert(result);
                            $.messager.progress('close');
    						if (result.errorMsg){
    							$.messager.show({
    	...
    	function addedata(){
    		...
    		$('#fmedata').form('clear');
    		url = '<?=$foldernya;?>save_edata.php[not_found]';
    	}
    	function editedata(index){
    		...
    		if (row){
                ...
                $('#fmedata').form('clear');
    			$('#fmedata').form('load',row); 
                url = '<?=$foldernya;?>update_edata.php[not_found]?id='+row.idedata;  
            }          
    	}
    	function saveedata(){
    	...
    	function destroyedata(index){
    		...
    		if (row){
    			$.messager.confirm('Konfirmasi','Yakin menghapus data ini?',function(r){
    				if (r){
    					$.post('<?=$foldernya;?>destroy_edata.php[not_found]',{id:row.idedata},function(result){
    						if (result.success){
    	...
    	function uploadtemplateedata(){
    		...
    		url = '<?=$foldernya;?>save_templateedata.php[not_found]';
    	}
