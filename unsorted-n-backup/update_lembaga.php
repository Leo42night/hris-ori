...
if ($userhris){    
    ...
    if($jwtToken!=""){
        ...
        $request -> setUrl($baseurl."/api/keanggotaanLembaga/save-batch");
        ...
        $rs31 = mysqli_query($koneksi,"select nip from r_lembaga where status_edit='1' group by nip order by nip asc");
        while($row1 = mysqli_fetch_array($rs31)){
            $nip = $row1['nip'];

            $datanya = "[";
            $i=1;
            $rs32 = mysqli_query($koneksi,"select * from r_lembaga where nip='$nip' order by id asc");
                ...
                $datanya .= '
                    {
                        ...
                        "endDate": "'.$end_date.'",
                        "nip": "'.$nip.'",
                        "namaOrganisasi": "'.$nama_organisasi.'",
                        "jabatan": "'.$jabatan.'",
                        "uraianKegiatan": "'.$uraian_kegiatan.'",
                        "kodeJenisOrganisasi": "'.$kode_organisasi.'",
            ...
            try {
                ...
                if (200 == $response->getStatus()) {
                    ...
                    if($statusnya==true){
                        $sql2 = "update r_lembaga set status_edit='0',tgl_edit='',user_edit='' where nip='$nip'";
            ...
            $sql1 .= " values('r_lembaga','$nip','','','$hari_ini','$userhris','$status','$keterangan')";
