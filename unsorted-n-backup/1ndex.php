...
if($jumlah_dashboard>0){
	$imgdashboard = "dashboard1.png";
} else {
	$imgdashboard = "dashboard1.png";
}
if($jumlah_kepegawaian>0){
...
if($jumlah_nonrutin>0){
	$imgnonrutin = "income1.png";
} else {
	$imgnonrutin = "income1.png";
}
if($jumlah_perpajakan>0){
...
if($jumlah_admin>0 || $userhris=="sandy"){
	$imgadmin = "admin1.png";
} else {
	$imgadmin = "admin1.png";
}
if($jumlah_hxms_api>0){
	$imghxmsapi = "api1.png";
} else {
	$imghxmsapi = "api1.png";
}
...
<script>
	...
	function showAll(){
		//$('#divmenu').accordion('showPanel', 1);//Module
		<?php if($jumlah_dashboard>0){?>
			$('#divmenu').accordion('showPanel', 1);//Dashboard
			$('#divmenu').accordion('getPanel',1).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 1);
		<?php } ?>
		<?php if($jumlah_kepegawaian>0){?>
			$('#divmenu').accordion('showPanel', 2);//Kepegawaian
			$('#divmenu').accordion('getPanel',2).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 2);
		<?php } ?>
		<?php if($jumlah_sppd>0){?>
			$('#divmenu').accordion('showPanel', 3);//Sppd
			$('#divmenu').accordion('getPanel',3).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 3);
		<?php } ?>
		<?php if($jumlah_cuti>0){?>
			$('#divmenu').accordion('showPanel', 4);//Cuti
			$('#divmenu').accordion('getPanel',4).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 4);
		<?php } ?>
		<?php if($jumlah_absensi>0){?>
			$('#divmenu').accordion('showPanel', 5);//Absensi
			$('#divmenu').accordion('getPanel',5).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 5);
		<?php } ?>
		<?php if($jumlah_konsumsi>0){?>
			$('#divmenu').accordion('showPanel', 6);//Konsumsi
			$('#divmenu').accordion('getPanel',6).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 6);
		<?php } ?>
		<?php if($jumlah_payroll>0){?>
			$('#divmenu').accordion('showPanel', 7);//Payroll
			$('#divmenu').accordion('getPanel',7).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 7);
		<?php } ?>
		<?php if($jumlah_nonrutin>0){?>
			$('#divmenu').accordion('showPanel', 8);//Non Rutin
			$('#divmenu').accordion('getPanel',8).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 8);
		<?php } ?>
		<?php if($jumlah_perpajakan>0){?>
			$('#divmenu').accordion('showPanel', 9);//Perpajakan
			$('#divmenu').accordion('getPanel',9).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 9);
		<?php } ?>	
		<?php if($jumlah_api>0){?>
			$('#divmenu').accordion('showPanel', 10);//API
			$('#divmenu').accordion('getPanel',10).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 10);
		<?php } ?>	
		<?php if($jumlah_master>0){?>
			$('#divmenu').accordion('showPanel', 11);//Master
			$('#divmenu').accordion('getPanel',11).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 11);
		<?php } ?>	
		<?php if($jumlah_admin>0){?>
			$('#divmenu').accordion('showPanel', 12);//Admin
			$('#divmenu').accordion('getPanel',12).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 12);
		<?php } ?>	
		<?php if($jumlah_hxms_api>0){?>
			$('#divmenu').accordion('showPanel', 13);//HXMS API
			$('#divmenu').accordion('getPanel',13).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 13);
		<?php } ?>	
		<?php if($jumlah_sap_api>0){?>
			$('#divmenu').accordion('showPanel', 14);//SAP API
			$('#divmenu').accordion('getPanel',14).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 14);
		<?php } ?>	
		<?php if($jumlah_pegawai>0){?>
			$('#divmenu').accordion('showPanel', 15);//Data Pegawai
			$('#divmenu').accordion('getPanel',15).panel('collapse');		
		<?php } else { ?>
			$('#divmenu').accordion('hidePanel', 15);
		<?php } ?>	
	}
...
$(document).ready(function() {	
	...
	//$('#divmenu').accordion('getPanel',2).panel('expand');
	/*
	$("#kd_pengadaanhistory").val("");
	$("#status_pengadaanhistory").val("");
	$("#kd_projecthistory").val("");
	$("#status_projecthistory").val("");
	$("#kd_tagihanhistory").val("");
	$("#status_tagihanhistory").val("");
	
    $('input:radio[name=cekhistory]').filter('[value=2]').prop('checked', true);
	$('input:radio[name="cekhistory"]').change(function(){
		var nilai_history = $(this).val();
		//alert(nilai_history);		
		$("#kodehistorycari").combobox('setValue','');
		$("#kodehistorycari").combobox('reload','get_kodehistorycari.php?jenis_history='+nilai_history);		
		reloadhistory(nilai_history);
		
	});	
	*/
	//$('#headerwest').css('background-image','url(images/new/hris.png)');
...
$(document).ready(function(){
	...
	*/    
	//$('#west').css('background','url(images/new/hris.png)');    
})
...
<!--
<div id="loading">
  <img id="loading-image" src="preloader3.gif" alt="Loading..." />
</div>
--> 
<!--
    ...
    <tr>        
        ...
        <td style="width: 70%; text-align: right; background-color: transparent !important;">
			...
			<div class="easyui-panel" style="text-align: right; border: none; background-color: transparent; padding-top:0px;">
				...
				<a href="#" class="easyui-menubutton" data-options="menu:'#mm1',iconCls:'fa fa-cog red'" style="border:none !important;background: transparent;margin-right:100px;text-decoration:none;"><span style="color:black;">ADMIN</span></a>
				<?php } ?>
        		<a href="#" class="easyui-menubutton" data-options="menu:'#mm2',iconCls:'fa fa-user blue'" style="border:none !important;background: transparent;text-decoration:none;"><span style="color:black;"><?=$namahris;?><span></a>
			</div>
			
			<div id="mm1" class="menu-content" style="width:230px;background: #e4f1ff;border:0 !important;">
				<div style="max-height:200px;overflow-y:scroll;">
				<?php include "menu_admin.php";?>
				</div>
			</div>
			
			<div id="mm2" class="menu-content" style="background: #e4f1ff;border:0 !important;">
...
<div id="west" data-options="region:'west',split:false,collapsible:false" title="<span style='font-size:20px;padding-left:3px;padding-right:3px;color:black;font-weight:bold;'>H R I S</span>" headerCls="headernya2" style="width:250px;padding:3px; background: transparent;overflow-y:scroll;overflow-x:hidden;">
	<div id="divmenu" class="easyui-accordion" data-options="multiple:true" style="width:100%;overflow:hidden;">
		<div title="<span style='font-size:9px;'>MAIN MENU</span><span style='font-size:9px;margin-left:30px;'><a href='javascript:void(0)' onclick='closeAllTab()' style='text-decoration:none;'>CLOSE ALL TABS</a></span>" data-options="collapsible:false" style="width:100%;overflow:auto;background:#fff;padding-right:3px;text-align:center;overflow:hidden;">
				...
				<tr>
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;">
						<a href='javascript:void(0)' title="DASHBOARD" style='text-decoration: none; color: black;' onclick='showdashboard()'>
							<div>
								<img src="images/new/<?=$imgdashboard;?>" style="height:40px;" />
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;">
						<a href='javascript:void(0)' title="KEPEGAWAIAN" style='text-decoration: none; color: black;' onclick='showkepegawaian()'>
							<div>
								<img src="images/new/<?=$imgkepegawaian;?>" style="height:40px;" />
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title=" REST-API" style='text-decoration: none; color: black;' onclick='showhxmsapi()'>
							<div>
								<img src="images/new/<?=$imgapi;?>" style="height:40px;" />
			...
			<hr style="borderWidth: 5vh;width: 100%;border-top: 1px dashed #757373;border-bottom: none;opacity: unset;"/>
			...
			<table style="width:100%;">
				<tr>
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="SPPD" style='text-decoration: none; color: black;' onclick='showsppd()'>
							<div>
								<img src="images/new/<?=$imgsppd;?>" style="height:40px;" />
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="ABSENSI" style='text-decoration: none; color: black;' onclick='showabsensi()'>
							<div>
								<img src="images/new/<?=$imgabsensi;?>" style="height:40px;" />
				...
				<tr>
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="CUTI" style='text-decoration: none; color: black;' onclick='showcuti()'>
							<div>
								<img src="images/new/<?=$imgcuti;?>" style="height:40px;" />
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="KONSUMSI" style='text-decoration: none; color: black;' onclick='showkonsumsi()'>
							<div>
								<img src="images/new/<?=$imgkonsumsi;?>" style="height:40px;" />
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="PAYROLL" style='text-decoration: none; color: black;' onclick='showpayroll()'>
							<div>
								<img src="images/new/<?=$imgpayroll;?>" style="height:40px;" />
				...
				<tr>
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="PERPAJAKAN" style='text-decoration: none; color: black;' onclick='showperpajakan()'>
							<div>
								<img src="images/new/<?=$imgperpajakan;?>" style="height:40px;" />
					...
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="MASTER" style='text-decoration: none; color: black;' onclick='showmaster()'>
							<div>
								<img src="images/new/<?=$imgmaster;?>" style="height:40px;" />
				...
				<!-- <tr>
					<td style="width:30%;padding:3px;text-align:center;vertical-align:top;padding-top:5px;">
						<a href='javascript:void(0)' title="ADMIN" style='text-decoration: none; color: black;' onclick='showadmin()'>
							<div>
								<img src="images/new/<?=$imgadmin;?>" style="height:40px;" />
								<br/><span style="font-size:9px;color:#069287;">ADMIN</span>
							</div>
						</a>
					</td>
					<td style="width:5%;">&nbsp;</td>	
		...
		<div id="divdashboard" title="<span style='color:#000;'>DASHBOARD</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_dashboard" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_dashboard" class="easyui-tree" data-options="url:'menu_dashboard.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div id="divkepegawaian" title="<span style='color:#000;'>KEPEGAWAIAN</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_kepegawaian" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_kepegawaian" class="easyui-tree" data-options="url:'menu_kepegawaian.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div id="divsppd" title="<span style='color:#000;'>SPPD</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_sppd" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_sppd" class="easyui-tree" data-options="url:'menu_sppd.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>CUTI</span>" style="overflow:auto;background:#fff;padding:5px;">
			<div id="menu_cuti" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_cuti" class="easyui-tree" data-options="url:'menu_cuti.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>ABSENSI</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_absensi" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_absensi" class="easyui-tree" data-options="url:'menu_absensi.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>KONSUMSI</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_konsumsi" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_konsumsi" class="easyui-tree" data-options="url:'menu_konsumsi.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>PAYROLL</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_payroll" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_payroll" class="easyui-tree" data-options="url:'menu_payroll.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>PERPAJAKAN</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_nonrutin" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_nonrutin" class="easyui-tree" data-options="url:'menu_nonrutin.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>PERPAJAKAN</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_perpajakan" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_perpajakan" class="easyui-tree" data-options="url:'menu_perpajakan.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>REST-API</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_api" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_api" class="easyui-tree" data-options="url:'menu_api.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>DATA MASTER</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_master" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_master" class="easyui-tree" data-options="url:'menu_master.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>ADMIN</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_admin" class="easyui-panel" style="padding:0px; background: transparent; border: none;overflow:hidden;">
				<ul id="tt2_admin" class="easyui-tree" data-options="url:'menu_admin.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>HXMS-API</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_hxms_api" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_hxms_api" class="easyui-tree" data-options="url:'menu_hxms_api.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>SAP-API</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_sap_api" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_sap_api" class="easyui-tree" data-options="url:'menu_sap_api.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
		<div title="<span style='color:#000;'>PEGAWAI</span>" style="overflow:auto;background:#fff;padding:10px;">
			<div id="menu_pegawai" class="easyui-panel" style="padding:0px; background: transparent; border: none;">
				<ul id="tt2_pegawai" class="easyui-tree" data-options="url:'menu_pegawai.php',method:'get',animate:true,lines:true"></ul>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
    $(function(){  
		/*
		var dg = $('#dghistorypengadaan');
		var pager = dg.datagrid('getPager');
		pager.pagination({showPageList:false,displayMsg:false,showPageInfo:false});	
		$('#tt').tabs({
		...
		$("#ft").hide();
		//$('#dghistorypengadaan').datagrid('load', 'get_history_pengadaan.php');
		*/
	});
</script>

<!--
<div data-options="region:'east',split:true,collapsed:false" title="<span style='color:white;font-weight:normal;font-size:12px;'>Proses Activity</span>" headerCls="headernya" style="width:220px;">
	<div id="divlable_history" style="color:#black;padding-top:3px;padding-bottom:5px;margin-bottom:5px;">
		<table>
		<tr><td>
			<label><input class="radionya" type="radio" name="cekhistory" value="1">Project Aktivity</label>
		</td></tr>
		<tr><td>
			<label><input class="radionya" type="radio" name="cekhistory" value="2">Procurement Aktivity</label>
		</td></tr>
		<tr><td>
			<label><input class="radionya" type="radio" name="cekhistory" value="3">Cash Out Aktivity</label>
		</td></tr>
		<tr><td>
			<label><input class="radionya" type="radio" name="cekhistory" value="4" checked>Cash In Aktivity</label>
		</td></tr>
		</table>
	</div>
	<div id="divhistory_pengadaan">
		<input type="hidden" id="kd_pengadaanhistory" name="kd_pengadaanhistory" />
		<input type="hidden" id="status_pengadaanhistory" name="status_pengadaanhistory" />
		<input type="hidden" id="kd_projecthistory" name="kd_pengadaanhistory" />
		<input type="hidden" id="status_projecthistory" name="status_pengadaanhistory" />
		<input type="hidden" id="kd_tagihanhistory" name="kd_pengadaanhistory" />
		<input type="hidden" id="status_tagihanhistory" name="status_pengadaanhistory" />
		
		<input class="easyui-combobox"
			id="kodehistorycari" editable="true"
			name="kodehistorycari" value=""
			style="padding: 2px; width: 200px;" 
			data-options="
				onChange:onSelectpengadaanhistorycari,
				url:'get_kodehistorycari.php',
				required:false,
				method:'get',
				valueField:'value',
				textField:'text',
		...
		<table id="dghistorypengadaan" title="" class="easyui-datagrid" style="width:100%;height:100%"	
				url="get_history_pengadaan.php" pageSize="20"        
				toolbar="#" pagination="false" nowrap="false" method="post"   
				rownumbers="false" fitColumns="true" singleSelect="true" footer="#ft"
				>
			<thead>
				<tr>
					<th field="uraianhistory" width="300px" align="left" halign="center" class="line-no" data-options="styler:styler1"><label id="judulhistory">PROCUREMENT ACTIVITY</label></th>
				</tr>
			</thead>
		</table>
	</div>
...
<!--<div data-options="region:'south',border:false" style="height: 25px; padding-left: 5px; padding-right: 5px; text-align: left; font-size: smaller; color: whitesmoke; padding-top:3px;background-image: url('images/hrisheader.png'); background-size: cover; background-repeat: no-repeat; background-position: center left;">-->
    <table style="width: 100%;">
    <tr>
        <td style="width:30%;white-space: nowrap; padding-left: 3px; color: black; text-align: left;font-size:10px;font-weight:bold;"> 
			<!--Copyright&copy;2022 - <strong>PLN TARAKAN-->
        </td>
        <td style="width:40%;white-space: nowrap; color: black; text-align: center;font-weight:bold;font-size:10px;">
			<!--TARAKAN ENTERPRISE ACTIVITY MANAGEMENT SOLUTIONS-->
		...
		<!--
        <td style="width:30%;white-space: nowrap; padding-left: 5px; color: #fff; text-align: left;">        
        </td>
        <td style="width:40%;white-space: nowrap; color: #fff; text-align: center;">
        Copyright&copy;2021 - <strong>PLN KALTIMRA</strong>
        </td>
        <td style="width:30%;white-space: nowrap; padding-right: 5px; color: #fff; text-align: right;">
        powered by : PLN TARAKAN
        </td>
		-->
    </tr>
    </table>
...
<!--<div id="center" data-options="region:'center'" style="background-image: url('images/bgawalteams.png'); background-size: cover; background-repeat: no-repeat; background-position: top left;">-->
	<div id="tt" class="easyui-tabs" pill="false" data-options="tools:'#tab-tools'" headerCls="headernya" style="width:100%;height:100%;">
	</div>
	<script type="text/javascript">
		var index = 0;
		/*
		...
		*/
		...
		function addPanelnya(filter){
			...
			//index=21;
            if ($('#tt').tabs('exists',namafilter)){
                $('#tt').tabs('select',namafilter);
				//var tab = $('#tt').tabs('getSelected');
				//tab.panel('refresh', urlfilter+"?proses="+prosesfilter+"&view="+viewfilter);
            } else {
    			$('#tt').tabs('add',{
    				title: namafilter,
                    href: urlfilter+"?proses="+prosesfilter+"&view="+viewfilter,
    				closable: true
    			});
            }
		}
		...
		function gantiPass(){
			index=51;
            if ($('#tt').tabs('exists','Ganti Password')){
                $('#tt').tabs('select','Ganti Password');
            } else {
    			$('#tt').tabs('add',{
...
.datagrid-header .datagrid-cell-over{
	font-weight: bold;
	background:green !important;
...
.headernya9{
	/*background:#e4f1ff;*/
	background:url('images/new/hris.png');
}
.headernya7{
	/*background:#e4f1ff;*/
	background:#f0f0f0;
...
/*
.numberbox .textbox-text .ratakanan{
	text-align: right;
}
*/
.accordion {
...
.loading__ring {
  ...
  height: 30px;
  margin: 6px;
  border: 4px solid #0bb8f0;
  border-radius: 50%;
  animation: loading__ring__animate 1.2s cubic-bezier(0.5, 0, 0.5, 1) infinite;
}

@keyframes loading__ring__animate {
  0% {
    transform: scale(0.5);
    opacity: 1;
  }
...
.loading__ring:nth-child(4) {
  animation-delay: 0s;
}

.loading__ring:nth-child(5) {
  animation-delay: 0.15s;
}
/*
.textbox .textbox-text .textbox-label{
	height:28px;
	line-height:28px;
...
.info:hover {
	...
	border-color: #48acc9;
	background: #5bc0de;
	background: -webkit-linear-gradient(top, #5bc0de 0, #6fcce8 100%);
	background: -moz-linear-gradient(top, #5bc0de 0, #6fcce8 100%);
	background: -o-linear-gradient(top, #5bc0de 0, #6fcce8 100%);
	background: linear-gradient(to bottom, #5bc0de 0, #6fcce8 100%);
	filter: progid:DXImageTransform.Microsoft.gradient(startColorstr=#5bc0de,endColorstr=#6fcce8,GradientType=0);
}
a.info:hover {
	background: #6fcce8;
	filter: none;
