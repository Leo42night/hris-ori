...
if ($userhris){
    ...
    //$request -> setUrl('https://10.1.85.223:7071/api/auth/login');
    //$request -> setUrl('https://10.1.88.221/api/auth/login');
    ...
    try {
        ...
        if (200 == $response->getStatus()) {
            ...
            $pesan = 'Unexpected HTTP status: ' . $response->getStatus() . ' ' .$response->getReasonPhrase() . ' ' .$response->getBody();
            //echo json_encode(array('errorMsg'=>$pesan));
        }
