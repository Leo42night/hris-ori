...
if ($userhris){    
    ...
    if($jwtToken!=""){
        ...
        $request -> setUrl($baseurl."/api/karya-tulis/save-batch");
        ...
        $rs31 = mysqli_query($koneksi,"select nip from r_karya_tulis where status_edit='1' group by nip order by nip asc");
        while($row1 = mysqli_fetch_array($rs31)){
            $nip = $row1['nip'];

            $datanya = "[";
            $i=1;
            $rs32 = mysqli_query($koneksi,"select * from r_karya_tulis where nip='$nip' order by id asc");
                ...
                $datanya .= '
                    {
                        ...
                        "nip": "'.$nip.'",
                        "judul": "'.$judul.'",
                        "mediaPublikasi": "'.$media_publikasi.'",
                        "tahun": "'.$tahun.'",
                        "kodeJenisKaryaTulis": "'.$kode_jenis.'",
            ...
            try {
                ...
                if (200 == $response->getStatus()) {
                    ...
                    if($statusnya==true){
                        $sql2 = "update r_karya_tulis set status_edit='0',tgl_edit='',user_edit='' where nip='$nip'";
            ...
            $sql1 .= " values('r_karya_tulis','$nip','','','$hari_ini','$userhris','$status','$keterangan')";
