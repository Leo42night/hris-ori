...
if ($userhris){
    ...
    $perintah = "";
    if($nama2!=""){
        $perintah .= " where nip='$nama2' or nama_lengkap like '%$nama2%'";
    }
    ...
    while ($hasil = mysqli_fetch_array($rs)) {
    	...
    	$nip = stripslashesx($hasil['nip']);
            $rs2 = mysqli_query($koneksi,"select * from data_pegawai where nip='$nip' and status_integrasi='1'");
            $hasil2 = mysqli_fetch_array($rs2);
            if($hasil2){
                $status_integrasi = stripslashesx($hasil2['status_integrasi']);
            } else {
                $status_integrasi = "0";
            }
        ...
        */
        ...
        if($no_kk!="" && strlen($nik<20)){
            $no_kk = decryptText($no_kk, $kunci);
        }
        if($npwp!="" && strlen($nik<20)){
            $npwp = decryptText($npwp, $kunci);
        }
        if($telepon!="" && strlen($nik<20)){
        ...
        $datanya["tgl_pegawaidatapegawai"] = $tgl_pegawai;
