...
if ($userhris){    
    ...
    if($jwtToken!=""){
        ...
        $request -> setUrl($baseurl."/api/keahlian/save-batch");
        ...
        $rs31 = mysqli_query($koneksi,"select nip from r_keahlian where status_edit='1' group by nip order by nip asc");
        while($row1 = mysqli_fetch_array($rs31)){
            $nip = $row1['nip'];

            $datanya = "[";
            $i=1;
            $rs32 = mysqli_query($koneksi,"select * from r_keahlian where nip='$nip' order by id asc");
                ...
                $datanya .= '
                    {
                        ...
                        "kodeTingkatKeahlian": "'.$tingkat_keahlian.'",
            ...
            try {
                ...
                if (200 == $response->getStatus()) {
                    ...
                    if($statusnya==true){
                        $sql2 = "update r_keahlian set status_edit='0',tgl_edit='',user_edit='' where nip='$nip'";
            ...
            $sql1 .= " values('r_keahlian','$nip','','','$hari_ini','$userhris','$status','$keterangan')";
