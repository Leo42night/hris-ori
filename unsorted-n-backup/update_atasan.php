...
if ($userhris){    
    ...
    if($jwtToken!=""){
        ...
        $request -> setUrl($baseurl."/api/riwayatAtasan/save");
        ...
        $i=1;
        $rs32 = mysqli_query($koneksi,"select * from r_atasan where status_edit='1' order by id asc");
        while($row = mysqli_fetch_array($rs32)){
            ...
            */
            ...
            $datanya .= '
                {
                    "startDateJabatan": "'.$start_date_jabatan.'", 
                    "startDate": "'.$start_date.'",
                    "endDate": "'.$end_date.'",
                    "nip": "'.$nip.'",
                    "nipAtasanLangsung": "'.$nip_atasan.'",
                    "jabatanAtasanLangsung": "'.$jabatan_atasan.'",
                    "namaAtasanLangsung": "'.$nama_atasan.'",
                    "kodePosisiAtasanLangsung": "'.$kode_posisi.'",
            ...
            try {
                ...
                if (200 == $response->getStatus()) {
                    ...
                    if($statusnya==true){
                        $sql2 = "update r_atasan set status_edit='0',tgl_edit='',user_edit='' where id=$id";
            ...
            $sql1 .= " values('r_atasan','$id','$tgl_edit','$user_edit','$hari_ini','$userhris','$status','$keterangan')";
